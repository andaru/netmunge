...
class Parser(object):
    """Netmunge's cachhing wrapper around Yapps grammars and parsers."""
    ...
    def get_grammar(self, vendor, command):
        ...
        """
        v = self._vendor.get(vendor)
        if v is None:
            for _v, _c_r in GRAMMARS.iterkeys():
                if vendor == _v:
                    if vendor not in self._vendor:
                        self._vendor[vendor] = {}
                    if _c_r not in self._regexes:
                        self._regexes[_c_r + '$'] = {}

                    self._vendor[vendor][_c_r + '$'] = GRAMMARS[(_v, _c_r)]
        ...
        if not v:
            ...
            for reg in self._regexes.iterkeys():
                ...
                if not re.compile(reg).match(command):
                    continue
                else:
